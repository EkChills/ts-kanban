...
const BoardOption = ({boardName}:{boardName:string}) => {
  ...
  return (
    <div onClick={() => dispatch(changeBoard(boardName))} className={`flex pl-[1.5rem] select-none cursor-pointer items-center space-x-[.75rem] py-[1rem] rounded-r-full ${selectedBoard.trim().toLowerCase() === boardName.trim().toLowerCase() ? 'bg-[#635FC7]' : 'bg-[none]'}`}>
